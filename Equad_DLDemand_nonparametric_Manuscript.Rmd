...
$$\hat m({\text{x}}) = {(I'K({\text{x}})I)^{ - 1}}I'K({\text{x}})y$$
  
where,  
  
$I$ is the identity matrix of dimension $n \times 1$,  
...
Conceptually, the LLLS estimator is similar to the LCLS with the exception that instead of fitting a constant to our left hand side variable we now consider fitting a locally defined line. This allows us to more accuratly model the data and is especially useful when we have outliers and nonlinear relationships in our data.  
