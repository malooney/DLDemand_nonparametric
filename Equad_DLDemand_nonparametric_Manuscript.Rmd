...
subtitle: "Recovering elasticities using nonparametric regression"
...
The LLLS estimator has been growing in popularity as an alternative to the LCLS method. In addition to increased use in applied research the LLLS estimator has gained a dominate position in the theoretical literature.  
...
After testing both bandwidth selection methods the method which provided the best fit was the Akaike Information Criterion Cross Validation (aic-CV). This method was passed to the nonparametric regression model.
  
The nonparametric regression techniques used in this study are the Local-Constant Least Squares and the Local-Linear Least Squares. With both of these techniques we estimated the residuals of the regression and the gradients, which are of interest when attempting to directly compare the elasticities obtained by the parametric regression versus the nonparametric regression.  
  
After testing both regression methods the method which provided the best fit was the Local-Linear Least Squares (LLLS). This method was used to obtain the data presented in table 3 (below).
...
Since the parametric form model we have chosen to work with only permits within equation constraints it only makes sense to make compare own price and income elasticities. When we compare the own price elasticities of the parametric regression to the elasticities obtained from the nonparametric regression we notice the nonparametric own price elasticities are all greater in magnitude. When we look at the income elasticities between the two regressions the nonparametric model has lower values. However, the numeric values are all very close and the signs are all in agreement for the elasticities that are significant.  
  
  Due to the mechanism of estimation of the nonparametric regression a one-for-one parameter comparison is complicated by the fact that nonparametric regressions make use of the full range of estimates as opposed to parametric regressions which make point estimates of elasticities, in other words, our nonparametric estimates are local while our parametric estimates are global. This means we can get a better sense of how the nonparametric elasticities are distributed across the data by plotting the gradients, in our case the actual elasticity estimates, and viewing the distribution of the elasticities across the entire sample range.  
  
The plots below show the distribution for the own price and income elasticities. It becomes clear that we have relatively tight clustering of own price elasticities for meats and dairy but we have an unusual distribution of own price elasticity on beans. We have a primary peak with two smaller peaks of elasticities to either side of the primary peak. This could mean that our sample population is not homogeneous in their preference for beans. While a positive own price elasticity is rare, it is theoretically possible. With the income elasticities we see relatively tight clustering for all three commodities.  
